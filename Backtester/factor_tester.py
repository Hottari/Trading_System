...
class FactorTester():
    ...
    def do_factor_test(self, factor, fr, rets, need_perf_columns=['CAGR(%)', 'Annual_Sharpe', 'MDD(%)']):
        ...
        df_perf = pd.concat([
            bt.perf_table(ret=ret_alpha[start:end], name='alpha', is_compound=True),
            bt.perf_table(ret=ret_ls[start:end], name='ls', is_compound=True),
            bt.perf_table(ret=rets['BTCUSDT'][start:end], name='btc', is_compound=True),
